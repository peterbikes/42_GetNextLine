...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/11/25 16:54:47 by psotto-m          #+#    #+#             */
/*   Updated: 2022/11/25 16:54:50 by psotto-m         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (0);
	fdarr = read_to_fdarr(fd, fdarr);
	if (!fdarr)
		return (NULL);
	print = line_or_eof(fdarr);
	fdarr = set_fdarr(fdarr);
	return (print);
}
